...
- ◐ **P2 — human-vs-AI attribution visual language + chips (done); graph theming + collapsible rail remain.** The differentiating pass (`styles/components.css`): **History** version rows carry the attribution — AI rows get a teal (`--ai`) left edge and an "AI" badge, human rows read "you"; **Diff** insertions/deletions show in the success/danger hues (deletes struck through); **tags** render as rounded chips (pressed = accent); and the **kept-vs-reverted** headline is coloured against the ≥~70% target (green good / red low). Markup gained only attribution hooks (`data-kind`, a `data-state`, a `tag-chips` class); all tests stay green. *Remaining P2:* theme the **graph** via tokens (it still uses hardcoded SVG hex, so dark mode is off there) and make the **rail panels collapsible**.
...
1. **UI/visual design** (workstream beyond the MVP polish bar; full plan in [`DESIGN.md`](DESIGN.md)). Decided (Phase 0): **3-pane** layout, **calm & dense** aesthetic, **CSS design tokens + lightweight CSS** (no framework), **Claude Design** as the design-system hub in Phase 3. Phases: ✅ **P1 tokens & theme** → ◐ **P2 layout & component pass** (3-pane shell + base styling + human-vs-AI attribution/diff/chips/kept-rate visual language done; **next: theme the graph via tokens** so dark mode is right, + a collapsible rail) → ▢ **P3 Claude Design** (`/design-sync` styleguide) → ▢ **P4 polish & web** (a11y, responsive; web reuses `@spherewiki/ui`).

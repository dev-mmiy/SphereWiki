...
1. **M5 — reliability hardening (continued).** Done: CRDT data-safety property tests (body convergence / no-lost-edits, note-list/registry convergence, revert round-trip) **and** the AI cross-workspace isolation guard. Remaining: an **onboarding** pass (first-run empty-vault UX, error/loading states) and, optionally, hook-level concurrent-reconcile stress (the CRDT layer is now property-covered, so lower priority). *Main local work left toward MVP exit.*

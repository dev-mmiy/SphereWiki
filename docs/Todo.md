...
- ◐ **P2 — 3-pane layout shell (done); per-component polish ongoing.** `NoteWorkspace` is restructured into the app shell: a top bar (brand · who · theme) over a CSS-grid **3 panes** — left **sidebar** (search + note list), center **`<main>` editor pane** (editor / empty state / AI bar / diff), right **rail** (Links, Tags, Metrics, Graph, History, Ask). `styles/workspace.css` gives the grid + surfaces + quiet panel headings + base controls (buttons / inputs / note-list items / selected state / editor frame / AI bar) from the tokens. Proper landmarks (header / main / two complementary asides / nav), single `h1`. All 318 tests stay green (restructure preserved roles/regions).
- ◐ **P2 — human-vs-AI attribution visual language + chips + graph theming (done); collapsible rail remains.** The differentiating pass (`styles/components.css`): **History** version rows carry the attribution — AI rows get a teal (`--ai`) left edge and an "AI" badge, human rows read "you"; **Diff** insertions/deletions show in the success/danger hues (deletes struck through); **tags** render as rounded chips (pressed = accent); and the **kept-vs-reverted** headline is coloured against the ≥~70% target (green good / red low). The **graph** is now token-driven too — node/edge/label colours come from CSS vars (`--muted`/`--accent`/`--surface`/`--text`/`--border`) instead of hardcoded SVG hex, so it follows light/dark; only geometry stays inline. Markup gained only attribution hooks (`data-kind`, a `data-state`, a `tag-chips` class); all tests stay green. *Remaining P2:* make the **rail panels collapsible**.
...
1. **UI/visual design** (workstream beyond the MVP polish bar; full plan in [`DESIGN.md`](DESIGN.md)). Decided (Phase 0): **3-pane** layout, **calm & dense** aesthetic, **CSS design tokens + lightweight CSS** (no framework), **Claude Design** as the design-system hub in Phase 3. Phases: ✅ **P1 tokens & theme** → ◐ **P2 layout & component pass** (3-pane shell + base styling + attribution/diff/chips/kept-rate + token-driven graph done; **next: a collapsible rail**) → ▢ **P3 Claude Design** (`/design-sync` styleguide) → ▢ **P4 polish & web** (a11y, responsive; web reuses `@spherewiki/ui`).

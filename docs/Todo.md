...
2. **Polish candidates (small, optional UX):** a sync loading/"syncing…" indicator (needs an injectable non-hydrating transport in `NoteWorkspace` to test); a richer first-run welcome; client-side room-auth token threading (inert until WorkOS, but readies the seam). *(Cmd-K quick switcher, `[[` wikilink autocomplete, AI suggest-mode review — done.)*

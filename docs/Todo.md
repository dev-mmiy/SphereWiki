...
1. **UI/visual design** (workstream beyond the MVP polish bar; full plan in [`DESIGN.md`](DESIGN.md)). Decided (Phase 0): **3-pane** layout, **calm & dense** aesthetic, **CSS design tokens + lightweight CSS** (no framework), **Claude Design** as the design-system hub in Phase 3. Phases: ✅ **P1 tokens & theme** → ✅ **P2 layout & component pass** (3-pane shell + base styling + attribution/diff/chips/kept-rate + token-driven graph + collapsible rail) → ▢ **P3 Claude Design** (`/design-sync` styleguide — **next**) → ▢ **P4 polish & web** (a11y, responsive; web reuses `@spherewiki/ui`).

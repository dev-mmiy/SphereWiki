...
1. **M5 — reliability hardening (continued).** Core CRDT data-safety property tests are in (body convergence / no-lost-edits, note-list/registry convergence, revert round-trip). Remaining: **isolation** property/guard tests (a workspace's notes / index / RAG never leak across workspaces — assert at the data layer, not just by construction); an **onboarding** pass (first-run empty-vault UX, error states); and optionally hook-level concurrent-reconcile stress (the CRDT layer is now property-covered, so this is lower priority). *Main local work left toward MVP exit.*

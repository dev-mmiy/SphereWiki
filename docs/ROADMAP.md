...
**M3b — Real WebSocket transport (done) & cloud control plane (deferred — credentials).** ✅ **Hocuspocus WebSocket** super-peer (`apps/server`) — two real clients converge over actual sockets (integration-tested), durability wired through the persistence seam. **Deferred (need credentials):** **WorkOS** AuthKit implementing the auth seam; **Cloud SQL + GCS** implementing the persistence seam; Postgres RLS for isolation; the desktop/web clients wired to the live server.
...
*Done:* ✅ on save, AI links/tags appear as attributed, revertible edits; retrieval is workspace-scoped and cited; `pnpm verify` green. Split out of M4 because the real backends need a model download + a Claude key.

**M4b — Real AI backends + wiring (deferred — credentials/download).** Real **multilingual-e5-small (ONNX)** embedder (model download + forced reindex), **pgvector** (server) / **DuckDB** (desktop) vector backends with Postgres RLS beneath the by-construction isolation, the **Claude-backed** suggester + answerer (BYO key via Secret Manager), the **server-side agent as a Hocuspocus CRDT peer**, and the desktop `aiOnSave` action wired into the editor — all behind the M4a seams, unchanged.

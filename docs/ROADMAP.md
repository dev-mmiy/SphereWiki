...
> - **Whole-vault sync (S4):** ✅ *S4a (done)* — an engine-agnostic **note-registry CRDT** (`CrdtRegistry` / `openYjsRegistry`, a per-workspace map of note id → `{title}`) so creating / renaming / removing a note can propagate to peers, distinct from each note's body `CrdtNote`. Convergent + idempotent + remote-flagged, with a read-boundary guard that drops malformed remote entries (Yjs does no runtime validation), all unit-tested. *S4b (next)* — wire the registry into the desktop note list and sync it over a workspace-level room so peers see each other's notes; multiplex one socket per workspace (the workspace is the replication boundary) instead of one per note.
